...
non_zero=np.count_nonzero(ratings_matrix)
global_mean=float(np.sum(ratings_matrix))/non_zero	#calculate global baseline
print(global_mean)
#calculate biases
b_users=[]
for user in range(len(ratings_matrix)):
	b_users.append(np.mean(ratings_matrix[user,:])-global_mean)

b_movies=[]
for movie in range(len(ratings_matrix[0])):
	b_movies.append(np.mean(ratings_matrix[:,movie])-global_mean)
print("Biases calculated")

predicted_preferred=[]
for neigbours in neigbour_list:
	print("Considering closest neigbours",neigbours)
	for test_user in range(0,len(train_matrix)/3):
		preferred_movies=[]
		for test_movie in range(0,len(train_matrix[0])/3):
			if(ratings_matrix[test_user][test_movie]!=0):
				#Choose to calculate only those ratings which were there before just to find
				#error
				
				
				#print("Predicting for", test_user,test_movie)
				...
				for closest_movie in ordered_neigbours:		
					...
					if(ratings_matrix[test_user][closest_movie]!=0):
						den_score+=score
						num_score+=(score*(ratings_matrix[test_user][closest_movie]-(global_mean+b_users[test_user]+b_movies[closest_movie])))
				...
				num_score+=(global_mean+b_users[test_user]+b_movies[test_movie])
